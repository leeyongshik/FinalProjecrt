...
public interface CartDAO extends JpaRepository<CartDTO, String> {
	...
	@Query("select cartDTO from CartDTO cartDTO where cartDTO.userName=:userName order by cart_seq desc")
	public List<CartDTO> findByCart(@Param("userName") String userName);
	
	@Query("select cartDTO.count from CartDTO cartDTO where cartDTO.userName=:userName order by cart_seq desc")
